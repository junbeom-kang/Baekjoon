...
def solution(n: int, p: int, e: int, reqs: list):
    for req in list(combinations(reqs, p)):
        print(reduce(lambda x, y: (print(x[0]),x[1]+y[1], x[2]+y[2]), req,(0,0,0)))

        _, small, big = reduce(lambda x, y: (0, x[1]+y[1], x[2]+y[2]), req, (0, 0, 0))
        #print(reduce(lambda x, y: (0, x[1]+y[1], x[2]+y[2]), req, (0, 0, 0)))
        #small, big = reduce(lambda x, y: (0,x[1]+y[1], x[2]+y[2]), req)
        print(req)
